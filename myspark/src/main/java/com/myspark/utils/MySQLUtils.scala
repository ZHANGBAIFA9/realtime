...
object MySQLUtils {
  ...
  def getMySQLInfo: (String, String, String) = {
    val jdbcURL = PropertiesInit.getFileProperties( "application.properties", "mysql.jdbc.url" )
    val userName = PropertiesInit.getFileProperties( "application.properties", "mysql.jdbc.username" )
    val passWord = PropertiesInit.getFileProperties( "application.properties", "mysql.jdbc.password" )
  ...
  */
  ...
  /**
    ...
    * @param updateColumns
    */
  def insertOrUpdateDFtoDB(tableName: String, resultDateFrame: DataFrame, updateColumns: Array[String]) {
    val colNumbers = resultDateFrame.columns.length
